...
- `draft` — готов к публикации (`draft_now` — опубликовать сразу, без даты);
...
Слоты: 07:30, 12:00, 18:00, 20:30 по Лондону. Pinterest читает время в CSV как UTC —
`build_csv.py` переводит сам (нужен пакет `tzdata`).

Цикл запускается в воскресенье и наполняет неделю **со вторника по понедельник** — один
день запаса всегда остаётся. Первый такой запуск: 27.09.2026 → 29.09–05.10.
